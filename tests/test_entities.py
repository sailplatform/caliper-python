...
class EntityTests(unittest.TestCase):
    ...
    def testAssessment(self):
        ...
        self.assertEqual(
            the_entity.as_json(thin_props=True,
                               thin_context=True),
            util.get_common_fixture(fixture))

    def testAssessmentItem(self):
        fixture = 'caliperEntityAssessmentItem'
        the_entity = util.build_assessment_items(
            assessment=util.build_assessment())[0]
        self.assertEqual(
            the_entity.as_json(thin_props=True,
                               thin_context=True),
            util.get_common_fixture(fixture))
    ...
    def testFillInBlankResponse(self):
        ...
        attempt = util.build_assessment_item_attempt(actor=student,
                                                     assessment=asmnt)
