...
class MinimalEventProfile(unittest.TestCase):
    ...
    def testCreatedMinimalEvent(self):
        ...
        self.assertEqual(minimal_event.as_json(), util.get_fixture(fixture))
...
class AnnotationProfile(unittest.TestCase):
    ...
    def testBookmarkAnnotationEvent(self):
        ...
        annotation_event = util.build_annotation_event(
            ...
            actor=self.student,
            annotation=util.build_bookmark_annotation(actor=self.student,
        ...
        self.assertEqual(annotation_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            annotation_event.as_json(thin_props=True,
                                     thin_context=True),
    ...
    def testHighlightAnnotationEvent(self):
        ...
        annotation_event = util.build_annotation_event(
            learning_context=self.learning_context,
            actor=self.student,
            annotation=util.build_highlight_annotation(actor=self.student,
                                                       annotated=event_object),
            event_object=event_object,
            index=1,
    ...
    def testSharedAnnotationEvent(self):
        ...
        annotation_event = util.build_annotation_event(
            learning_context=self.learning_context,
            actor=self.student,
            annotation=util.build_shared_annotation(actor=self.student,
                                                    annotated=event_object),
            event_object=event_object,
            index=3,
    ...
    def testTagAnnotationEvent(self):
        ...
        annotation_event = util.build_annotation_event(
            learning_context=self.learning_context,
            actor=self.student,
            annotation=util.build_tag_annotation(actor=self.student,
                                                 annotated=event_object),
            event_object=event_object,
            index=4,
...
class AssessmentProfile(unittest.TestCase):
    def setUp(self):
        ...
        self.assessment_item = util.build_assessment_items(
            assessment=self.assessment)[0]
        self.attempt = util.build_assessment_attempt(
            actor=self.student, assessment=self.assessment)
        self.item_attempt = util.build_assessment_item_attempt(
            actor=self.student, assessment=self.assessment)
    ...
    def testAssessmentEvent(self):
        ...
        self.assertEqual(assessment_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            assessment_event.as_json(thin_props=True,
                                     thin_context=True),
            util.get_common_fixture(fixture))
    ...
    def testAssessmentItemCompletedEvent(self):
        ...
        assessment_item_event = util.build_assessment_item_event(
            ...
            actor=self.student,
            assessment_item=self.assessment_item,
            generated=util.build_assessment_item_response(
                assessment=self.assessment,
...
class AssignableProfile(unittest.TestCase):
    ...
    def testAssignableEvent(self):
        ...
        self.assertEqual(assignable_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            assignable_event.as_json(thin_props=True,
                                     thin_context=True),
            util.get_common_fixture(fixture))
...
class MediaProfile(unittest.TestCase):
    ...
    def testVideoMediaEvent(self):
        ...
        self.assertEqual(video_media_event.as_json(),
                         util.get_fixture(fixture))
        self.assertEqual(
            video_media_event.as_json(thin_props=True,
                                      thin_context=True),
            util.get_common_fixture(fixture))
...
class OutcomeProfile(unittest.TestCase):
    def setUp(self):
        ...
        self.attempt = util.build_assessment_attempt(
            actor=self.student, assessment=self.assessment)
        self.result = util.build_assessment_result(
            learning_context=self.learning_context,
            actor=self.student,
            attempt=self.attempt)
    ...
    def testAssessmentOutcomeEvent(self):
        ...
        self.assertEqual(assessment_outcome_event.as_json(),
                         util.get_fixture(fixture))
        self.assertEqual(
            assessment_outcome_event.as_json(thin_props=True,
                                             thin_context=True),
            util.get_common_fixture(fixture))
...
class ReadingProfile(unittest.TestCase):
    ...
    def testNavigationEvent(self):
        ...
        self.assertEqual(navigation_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            navigation_event.as_json(thin_props=True,
                                     thin_context=True),
            util.get_common_fixture(fixture))
    ...
    def testViewEvent(self):
        ...
        self.assertEqual(reading_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            reading_event.as_json(thin_props=True,
                                  thin_context=True),
            util.get_common_fixture(fixture))
    ...
    def testViewEventCoerced(self):
        ...
        util.put_fixture(fixture, reading_event)
        self.assertEqual(
            reading_event.as_json(
                thin_props=True,
                thin_context=True,
                described_entities=[
                    self.student.id, self.learning_context.edApp.id,
                    self.learning_context.group.subOrganizationOf.id
                ]),
            util.get_local_fixture(fixture))

    def testViewEventExtended(self):
        fixture = 'caliperEventViewViewedExtended'
        job = util.build_epub_view_event_extensions()
        reading_event = util.build_epub_view_event(
            learning_context=self.learning_context,
            actor=self.student,
            event_object=self.epub,
...
class SessionProfile(unittest.TestCase):
    ...
    def testSessionLoginEvent(self):
        ...
        self.assertEqual(session_event.as_json(), util.get_fixture(fixture))
        self.assertEqual(
            session_event.as_json(thin_props=True,
                                  thin_context=True),
            util.get_common_fixture(fixture))

    def testSessionLogoutEvent(self):
        fixture = 'caliperEventSessionLoggedOut'
        ctxt = util.build_readium_app_learning_context(
            actor=self.student, session=self.session_end)
